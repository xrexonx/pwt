    ...
    return html`
      <style>
        :host {
          ...
          color: var(--exmg-paper-toolbar-text-color, rgba(0, 0, 0, 0.87));
          font-size: 16px;
        }
        #selected-toolbar {
          padding: 0px 24px;
        ...
        #selected-toolbar {
          ...
          @apply --layout-justified;
          background: var(
            --exmg-paper-toolbar-selection-background-color,
            #e8effd
          );
      ...
      <div
        id="selected-toolbar"
        data-visible$="[[_isSelectedToolbarVisible(_selectedRows)]]"
      >
        <div class="title-container">
          <span>[[_selectedRows]]</span> item<template
            is="dom-if"
            if="[[_multipleItemsSelected(_selectedRows)]]"
            >s</template
          >
          selected
        </div>
...
window.customElements.define(
  ExmgPaperToolbarElement.is,
  ExmgPaperToolbarElement
);
