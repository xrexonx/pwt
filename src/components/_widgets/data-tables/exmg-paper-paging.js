...
export class ExmgPaperPagingElement extends GestureEventListeners(
  PolymerElement
) {
  static get template() {
    return html`
      <style>
        :host {
          ...
          color: var(--exmg-paper-paging-text-color, rgba(0, 0, 0, 0.87));
          font-size: 12px;
          font-weight: 500;
          height: 56px;
          padding: 0px 6px;
          ...
          --paper-dropdown-menu-icon: {
            color: var(--exmg-paper-paging-icon-color, rgba(0, 0, 0, 0.54));
          }
          --paper-icon-button: {
            color: var(--exmg-paper-paging-icon-color, rgba(0, 0, 0, 0.54));
            padding: 0;
          ...
          --paper-icon-button-disabled: {
            color: var(
              --exmg-paper-paging-icon-disabled-color,
              rgba(0, 0, 0, 0.36)
            );
          }
      ...
      <div class="page-actions">
        <paper-icon-button
          icon="exmg-paper-icons:first-page"
          on-tap="firstPage"
          disabled="[[_isFirstPage(pageIndex)]]"
        ></paper-icon-button>
        <paper-icon-button
          icon="exmg-paper-icons:chevron-left"
          on-tap="previousPage"
          disabled="[[_isFirstPage(pageIndex)]]"
        ></paper-icon-button>
        <paper-icon-button
          icon="exmg-paper-icons:chevron-right"
          on-tap="nextPage"
          disabled="[[_isLastPage(pageIndex, pageSize, totalItems)]]"
        ></paper-icon-button>
        <paper-icon-button
          class="last-page-button"
          icon="exmg-paper-icons:last-page"
          on-tap="lastPage"
          disabled="[[_isLastPage(pageIndex, pageSize, totalItems)]]"
        ></paper-icon-button>
      </div>
  ...
  static get observers() {
    return ['_observePageSizeList(_pageSize)', '_observePageSize(pageSize)'];
  }
